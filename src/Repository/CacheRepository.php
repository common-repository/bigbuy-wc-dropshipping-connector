...
class CacheRepository
{
    ...
    public function findOneById(string $itemId, string $ttlInterval): ?string
    {
        ...
        $sql = 'SELECT item_data FROM '.$this->tableName.' WHERE item_id = "'.$itemId.'" AND item_expiration_timestamp > (UNIX_TIMESTAMP(CURRENT_TIMESTAMP - INTERVAL '.$ttlInterval.')) OR item_expiration_timestamp = 0 ORDER BY date_add DESC';
