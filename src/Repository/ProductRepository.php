...
class ProductRepository
{
    ...
    /**
     * @param int[] $productShopIds
     *
     * @return array<int, int>
     */
    public function findByProductShopIdsIndexedByProductShopId(array $productShopIds): array
    {
     ...
     * @return array<int, int>
     */
    public function findProductShopIdsDisabledByProductShopIdIndexedByProductShopId(array $productShopIds): array
    {
        if (empty($productShopIds)) {
            return [];
        }
     ...
     */
    public function findProductShopIdsIndexedBySku(array $productSkus): array
    {
        if (empty($productSkus)) {
            return [];
        }

        $sql = 'SELECT product_id, sku FROM '.$this->tableName.' WHERE sku IN ("'.implode('","', $productSkus).'")';
        $result = $this->wpDb->get_results($sql, ARRAY_A);
    ...
    public function getImagesNotAssociateToProductByImageTitles(array $imageTitles): array
    {
        ...
        $imageTitleConditions = [];

        foreach ($imageTitles as $imageTitle) {
            $imageTitleConditions[] = 'post_title LIKE "'.\trim($imageTitle, '%').'%"';
        }

        $imageTitlesCondition = \implode(' OR ', $imageTitleConditions);

        $sql = 'SELECT ID FROM '.$this->postTable.' WHERE ('.$imageTitlesCondition.') AND post_parent = 0 AND post_type = '.self::POST_ATTACHMENT_TYPE.' AND post_mime_type LIKE "image/%";';
     ...
     */
    public function getImagesAssociateAndNotToProductByImageTitles(array $imageTitles, int $postIdToDelete): array
    {
        if (empty($imageTitles)) {
            return [];
        }
        ...
        $sql = 'SELECT ID FROM '.$this->postTable.' WHERE ('.$imageTitlesCondition.') AND (post_parent = 0 OR post_parent = '.$postIdToDelete.') AND post_type = '.self::POST_ATTACHMENT_TYPE.' AND post_mime_type LIKE "image/%";';
