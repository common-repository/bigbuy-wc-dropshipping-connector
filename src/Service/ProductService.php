...
class ProductService
{
    ...
    public function processProductByBatch(array $productsIndexedByProductMapId, array $products, int $fileId, string $languageIsoCode): array
    {
        ...
        $productsDataIndexedByProductMapId = array_chunk($productsIndexedByProductMapId, $this->systemService->getBatchValue(), true);

        foreach ($productsDataIndexedByProductMapId as $productDataIndexedByProductMapId) {
            $productFactorySave = [];
            ...
            $productShopIdsIndexedByProductMapId = $this->productMapManager->findProductShopIdsByProductMapIdsIndexedByProductMapId(\array_keys($productDataIndexedByProductMapId));

            $productShopIdsForProductTags = \array_values($productShopIdsIndexedByProductMapId);
            $productTagsGroupedByProductShopIds = $this->tagService->getProductTagsGroupedByProductShopIds($productShopIdsForProductTags);
            unset($productShopIdsForProductTags);

            $productTagIdsIndexedByMapIds = $this->tagService->findByTagShopIdsIndexedByTagMapIds($productTagsGroupedByProductShopIds);
            $productVersionsIndexedByProductMapId = $this->productMapManager->findVersionsByProductMapIdIndexedByProductMapId(\array_keys($productShopIdsIndexedByProductMapId));
            $processedProducts = $this->importProcessManager->getProcessedProducts(\array_keys($productShopIdsIndexedByProductMapId), $fileId);

            $productSkuIndexedByProductIds = \array_column($products, 'ProductID', 'SKU');
            $productsNotMappedIndexedBySku = $this->productManager->findProductShopIdsIndexedBySku(\array_keys($productSkuIndexedByProductIds));
            ...
            if (!empty($productShopIdsIndexedByProductMapId)) {
                $productShopIdsForExisting = \array_values($productShopIdsIndexedByProductMapId);
                $productShopIds = $this->productManager->findByProductShopIdsIndexedByProductShopId($productShopIdsForExisting);
                unset($productShopIdsForExisting);
            }
            ...
            foreach ($productDataIndexedByProductMapId as $productMapId => $productMapData) {
                if (\array_key_exists($productMapId, $processedProducts) && array_key_exists($productMapId, $productShopIdsIndexedByProductMapId)) {
                    $processedArray[][] = ['shopId' => (int)$productShopIdsIndexedByProductMapId[$productMapId], 'id' => (int)$productMapId];
                ...
                if (\array_key_exists($productMapId, $productShopIdsIndexedByProductMapId)) {
                    if (\array_key_exists($productMapData->sku, $productsNotMappedIndexedBySku)) {
                        $version = \json_decode($productVersionsIndexedByProductMapId[$productMapId], true);
                        ...
                        if (
                            \array_key_exists($productMapData->id, $productTagsGroupedByProductShopIds) && $productTagsGroupedByProductShopIds[$productMapData->id]
                            && array_key_exists($productMapData->id, $productTagIdsIndexedByMapIds) && $productTagIdsIndexedByMapIds[$productMapData->id]
                        ) {
                            $customerTags = array_diff($productTagsGroupedByProductShopIds[$productMapData->id], $productTagIdsIndexedByMapIds[$productMapData->id]);
    ...
    {
        if (empty($productIdsDisabledToDelete)) {
            $this->logger->getInstance()->info('No products to delete');
        ...
        try {
            foreach ($productIdsDisabledToDelete as $productIdDisabledToDelete) {
                $this->deleteImagesAssociateAndNotToProduct((int)$productIdDisabledToDelete);
            }
    ...
    public function deleteProductImageAttachment(string $productSku): void
    {
        ...
        if ($imageProductPosts) {
            $rootDir = $this->directoryService->getRootDir();

            foreach ($imageProductPosts as $imagePost) {
                \wp_delete_post($imagePost['ID'], true);

                $filepathData = \explode('/wp-content/', $imagePost['guid']);
                $filepath = null;

                if (!empty($filepathData[1])) {
                    $filepath = $rootDir.'/wp-content/'.$filepathData[1];
                }
    ...
    public function getProductSkuIndexedByProductId(array $productsProcessedFromFile): array
    {
        ...
        $productShopIdIndexedByProductMapId = $this->productMapManager->findProductShopIdsByProductMapIdsIndexedByProductMapId(\array_values($productsProcessedFromFile));
    ...
    private function deleteImagesAssociateAndNotToProduct(int $productShopId): void
    {
        ...
        $imageTitles = $this->sanitizeImageTitles($productImagesTitles);
        $imagesIdsAssociateAndNotToProduct = $this->productManager->getImagesAssociateAndNotToProductByImageTitles(\array_unique($imageTitles), $productShopId);

        if (empty($imagesIdsAssociateAndNotToProduct)) {
            return;
        }

        foreach ($imagesIdsAssociateAndNotToProduct as $imageIdToDelete) {
    ...
    public function deleteImagesNotAssociateToProducts(int $productShopId): void
    {
        ...
        $imageTitles = $this->sanitizeImageTitles($productImagesTitles);
        $imagesIdsNotAssociateToProduct = $this->productManager->getImagesNotAssociateToProductByImageTitles(\array_unique($imageTitles));
        ...
        $imageIdsToDelete = \array_diff_key($imagesIdsNotAssociateToProduct, $imageIdsWithProductMeta);
    ...
    /**
     * @param array<int, string> $productImagesTitles
     */
    private function sanitizeImageTitles(array $productImagesTitles): array
    {
        $imageTitles = [];
