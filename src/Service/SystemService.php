...
class SystemService
{
    ...
    private const REQUIRED_FUNCTION_NAMES = ['disk_free_space', 'disk_total_space', 'file_get_contents', 'glob'];
    ...
    public function checkFileFolderPermissions(): bool
    {
        ...
        try {
            $isWritableLogDir = @is_writable($logDir) && @is_readable($logDir);
        } catch (\Throwable $exception) {
            $isWritableLogDir = false;
            $this->loggerService->getInstance()->debug($exception->getMessage());
        }

        try {
            $isWritableFilesDir = @is_writable($importFilesDir) && @is_readable($importFilesDir);
        } catch (\Throwable $exception) {
            $isWritableFilesDir = false;
            $this->loggerService->getInstance()->debug($exception->getMessage());
        }

        try {
            $isWritableUploadsDir = @is_writable($uploadsDir) && @is_readable($uploadsDir);
        } catch (\Throwable $exception) {
            $isWritableUploadsDir = false;
            $this->loggerService->getInstance()->debug($exception->getMessage());
        }

        return $isWritableLogDir && $isWritableFilesDir && $isWritableUploadsDir;
    }
